...
''' trainer:
Train the designed CNN and evaluate using cross-validation.

Parameters:
    - eval_type: type of cross-validation (VALID_METHOD_KFOLD or VALID_METHOD_SUBJECT)
    - dataset_type: which dataset the training data belongs to (DATA_MMI or DATA_TT)
    - input_type: the type of content of the input images (INPUT_FULLFACE or INPUT_EYES)
...
'''
def trainer(eval_type, dataset_type, input_type, validation_size, nfolds, batch_size, num_iter, classes, data_path):
    ...
    if eval_type==dataset.VALID_METHOD_KFOLD:
        ...
        data = dataset.read_train_sets(train_path, img_size, classes, validation_size=validation_size, nfolds=nfolds, input_type=input_type)
        ...
        data = dataset.read_train_sets_persubject(train_path, img_size, classes, validation_size=validation_size, nsubjects=nfolds, input_type=input_type, dataset_type = dataset_type)
    else:
        ...
        def train(num_iteration):
            ...
            for i in range(total_iterations,
                ...
                if i % int(data.train.num_examples[j]/batch_size) == 0: 
                    ...
                    saver.save(session, data_path+'models/face-exp-model{}'.format(j)) 
